...
void tm1637_init(void)
{
    ...
    GPIO_Initure.Pin = TM_SCL_PIN;             // PB8
    GPIO_Initure.Mode = GPIO_MODE_OUTPUT_OD;   // 推挽输出
    GPIO_Initure.Pull = GPIO_NOPULL;           // 上拉
    GPIO_Initure.Speed = GPIO_SPEED_FREQ_HIGH; // 高速
    HAL_GPIO_Init(TM_SCL_PORT, &GPIO_Initure);

    GPIO_Initure.Pin = TM_SDA_PIN; // PD2
    HAL_GPIO_Init(TM_SDA_PORT, &GPIO_Initure);

    TM_SCL_HIGH(); // PA8置1，默认初始化后灯灭
    TM_SDA_HIGH(); // PD2置1，默认初始化后灯灭
